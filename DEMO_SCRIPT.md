...
- The deployed Vercel app reads the environment through `NEXT_PUBLIC_APP_URL` and uses whatever DKG node is configured by `DKG_ENDPOINT` / `DKG_PORT`. Verify against the environment that is actually running.
