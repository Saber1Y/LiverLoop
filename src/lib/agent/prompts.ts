...
- When the brief demands the CTA at the TOP of the frame (e.g. "top", "upper", "at the top", "top center"), set the CTA step's position to "top". A top-positioned CTA is rendered as large white text with a dark outline anchored to the top edge of the frame (ffmpeg-burn-subtitles cannot do that; the assembly layer handles it). When the brief is silent about placement, default to position "bottom".
...
- For a video brief with background audio AND a burned CTA, the canonical assembly order is: 1) the generated video clip step(s), 2) an ffmpeg-concat ONLY if there are multiple clips, 3) the audio/music step, 4) ffmpeg-mux (consumes the concat/clip output + the audio output), 5) ffmpeg-burn-subtitles (consumes the mux output) for the CTA, 6) ffmpeg-trim LAST with {"start_sec":0,"duration_sec":<exact brief duration>}. ffmpeg-mux and ffmpeg-burn-subtitles must come BEFORE ffmpeg-trim, never after it.
