      ...
      <div className="relative z-10 mx-auto max-w-[1400px]">
        ...
        <section className="grid gap-5 lg:grid-cols-[0.78fr_1.22fr]">
          ...
          <motion.div layout className="rounded-2xl border border-border bg-card/45 p-5 shadow-2xl shadow-black/20 backdrop-blur-xl md:p-7">
            ...
            {latestArtifact ? <ArtifactPreview artifact={latestArtifact} /> : <EmptyArtifact active={isActive} />}
          </motion.div>
        </section>

        {latest?.evaluation ? <EvaluationPanel evaluation={latest.evaluation} versions={data.versions} /> : <div className="mt-5 flex items-center gap-3 rounded-xl border border-dashed border-border px-5 py-4 font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground"><Sparkles className="size-3.5 text-primary" /> Critic review will appear after artifact completion.</div>}
      </div>
    </main>
  );
}
...
function Header({ runId }: { runId: string }) {
  return <header className="flex flex-wrap items-center justify-between gap-4 rounded-full border border-border bg-card/70 px-4 py-3 shadow-xl shadow-black/10 backdrop-blur-xl md:px-5"><Link href="/" className="flex items-center gap-3 font-mono text-sm tracking-[0.16em]"><span className="flex size-7 items-center justify-center rounded-md bg-primary text-primary-foreground"><span className="size-2 rounded-full bg-current" /></span>LIVERLOOP</Link><div className="flex items-center gap-4"><Link href={`/provenance/${runId}`} className="text-xs text-muted-foreground transition-colors hover:text-foreground">Provenance</Link><Link href="/knowledge" className="text-xs text-muted-foreground transition-colors hover:text-foreground">Knowledge</Link><Link href="/run/new" className="inline-flex items-center gap-2 rounded-full border border-border px-3 py-1.5 text-xs text-muted-foreground hover:text-foreground"><ArrowLeft className="size-3" /> New run</Link></div></header>;
}
...
function EmptyArtifact({ active }: { active: boolean }) { return <div className="flex min-h-[360px] items-center justify-center rounded-xl border border-dashed border-border"><div className="text-center">{active ? <LoaderCircle className="mx-auto size-6 animate-spin text-primary" /> : <CircleDashed className="mx-auto size-6 text-muted-foreground" />}<p className="mt-4 font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">{active ? "Waiting for Livepeer artifact" : "No artifact returned"}</p></div></div>; }
